...
SELECT 
	...
	(SELECT hometown FROM profiles WHERE user_id = users.id) AS town,
	(SELECT filename FROM media WHERE id = 
			(SELECT photo_id FROM profiles WHERE user_id = users.id)) AS photo
FROM users;

SELECT m.filename FROM media m
WHERE m.user_id = (SELECT u.id FROM users u WHERE u.email = 'arlo50@example.org')
AND m.media_type_id = (SELECT mt.id FROM media_types mt WHERE mt.name_type LIKE '%photo%');
